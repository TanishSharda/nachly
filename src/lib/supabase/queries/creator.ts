...
/**
 * Approve choreographer application (admin)
 */
export async function approveChoreographerApplication(
  ...
  if (appError) {
    console.error('[approveChoreographerApplication] Error:', appError);
    return { application: null, error: appError };
  }

  // Update user role to choreographer
  ...
  if (roleError) {
    console.error('[approveChoreographerApplication] Role update error:', roleError);
    return { application, error: roleError };
  }
...
/**
 * Reject choreographer application (admin)
 */
export async function rejectChoreographerApplication(
  applicationId: string,
  reason: string
) {
  const supabase = createServerSupabase();
  ...
  if (error) {
    console.error('[rejectChoreographerApplication] Error:', error);
    return { application: null, error };
  }
...
/**
 * Request revision for choreographer application (admin)
 */
export async function requestApplicationRevision(
  ...
  if (error) {
    console.error('[requestApplicationRevision] Error:', error);
    return { application: null, error };
  }
