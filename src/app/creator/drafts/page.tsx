  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Link href="/creator/upload" className="rounded-xl bg-gradient-to-r from-[#F3B2AB] to-[#D88B80] px-4 py-2 text-xs font-bold text-[#0a0a0a]">
          New Upload
        </Link>
      </div>

      {error && <div className="rounded-xl border border-red-400/20 bg-red-500/10 px-4 py-2 text-sm text-red-200">{error}</div>}

      <div className="grid gap-3 sm:grid-cols-2">
        {loading && <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-sm text-zinc-400">Loading drafts...</div>}
        {!loading && drafts.length === 0 && (
          <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] p-8 text-center text-sm text-zinc-400">No drafts yet. Start a new upload to create one.</div>
        )}
        {drafts.map((draft) => (
          <div key={draft.id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex flex-col gap-2">
                <Link href={`/creator/upload?draft=${draft.id}`} className="rounded-lg border border-white/10 px-3 py-1.5 text-[11px] text-zinc-200 hover:bg-white/5">
                  Continue
                </Link>
                <button type="button" onClick={() => deleteDraft(draft)} disabled={deletingId === draft.id} className="rounded-lg border border-red-400/20 px-3 py-1.5 text-[11px] text-red-300 hover:bg-red-500/10 disabled:opacity-40">
