    ...
    <motion.div initial="hidden" animate="visible" variants={stagger}>
      ...
      <motion.div variants={fadeUp} className="mb-6">
        <Card className="!p-0 overflow-hidden">
          <div className="h-32 bg-gradient-to-r from-[#344400] via-[#556d00] to-[#7b9e00] relative">
        ...
        <Card>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="How learners will see you" className="w-full rounded-xl border border-white/10 bg-zinc-900 px-4 py-2.5 text-sm text-white placeholder-zinc-600 outline-none focus:border-[#c4ff00]/40 transition" />
            </div>
            <div>
              <label className="text-xs font-medium text-zinc-400 mb-1.5 block">Bio</label>
              <textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={3} placeholder="Tell learners about yourself and your dance journey..." className="w-full rounded-xl border border-white/10 bg-zinc-900 px-4 py-2.5 text-sm text-white placeholder-zinc-600 outline-none focus:border-[#c4ff00]/40 transition resize-none" />
            ...
            <div>
              ...
              <textarea value={philosophy} onChange={(e) => setPhilosophy(e.target.value)} rows={2} placeholder="What&apos;s your approach to teaching dance?" className="w-full rounded-xl border border-white/10 bg-zinc-900 px-4 py-2.5 text-sm text-white placeholder-zinc-600 outline-none focus:border-[#c4ff00]/40 transition resize-none" />
            </div>
          </div>
        </Card>
      </motion.div>
        ...
        <Card>
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            {EXP_LEVELS.map((level) => (
              <button key={level.value} type="button" onClick={() => setExpLevel(level.value)} className={`rounded-xl border p-4 text-left transition ${expLevel === level.value ? "border-[#c4ff00]/40 bg-[#c4ff00]/10" : "border-white/10 bg-white/[0.03] hover:border-white/20"}`}>
                <p className={`text-sm font-semibold ${expLevel === level.value ? "text-[#c4ff00]" : "text-white"}`}>{level.label}</p>
        ...
        <Card>
          ...
          <div className="flex flex-wrap gap-2">
            {DANCE_STYLES.map((style) => (
              <button key={style} type="button" onClick={() => toggleStyle(style)} className={`rounded-xl px-4 py-2 text-sm font-medium transition ${selectedStyles.includes(style) ? "bg-[#c4ff00]/20 text-[#c4ff00] border border-[#c4ff00]/30" : "bg-white/5 text-zinc-400 border border-white/10 hover:border-white/20"}`}>
                {style}
              </button>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={social.label}>
                ...
                <input value={social.value} onChange={(e) => social.onChange(e.target.value)} placeholder={social.placeholder} className="w-full rounded-xl border border-white/10 bg-zinc-900 px-4 py-2.5 text-sm text-white placeholder-zinc-600 outline-none focus:border-[#c4ff00]/40 transition" />
              </div>
            ))}
          </div>
        </Card>
