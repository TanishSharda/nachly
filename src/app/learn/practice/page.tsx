  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#efe5d4_0%,#fbf8f2_32%,#f6f0e5_100%)] px-4 py-8 text-[#241811] sm:px-6 md:px-8">
      <div className="mx-auto max-w-4xl rounded-3xl border border-[#6c51321f] bg-white/75 p-6 backdrop-blur-xl sm:p-8">
        ...
        <h1 className="mt-2 text-3xl font-black tracking-tight text-[#2a1c11] sm:text-4xl">Open camera practice in one tap.</h1>
        <p className="mt-3 text-sm text-[#665543]">
          Pick a choreography and jump directly into the recording studio.
        </p>
        ...
        {posts.length > 0 ? (
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {posts.slice(0, 6).map((post: any) => (
              <Link
                key={post.id}
                href={`/record/${encodeURIComponent(post.id)}?mode=practice`}
              ...
              >
                ...
                <p className="mt-1 text-xs text-[#6d5e50]">Open Practice Studio</p>
              </Link>
            ))}
          </div>
        ) : (
