...
export async function GET(request: Request) {
  ...
  try {
    const maybe = await enforceRateLimit(request as any as NextRequest, { windowMs: 60_000, max: 30, keyPrefix: 'drills:progression' });
  ...
  // Use the request-scoped server client to ensure operations respect auth.
  const db = supabase;
